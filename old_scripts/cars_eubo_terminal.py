...
for t in range(1, NUM_QUERIES + 1):
    ...
    if torch.all(next_X[0] == next_X[1]):
        ...
        # keep only cars different from A
        mask = ~(all_cars == next_X[0]).all(dim=1)
        candidates = all_cars[mask]

        # simple fallback: pick a random different car for B
        idx = torch.randint(0, candidates.size(0), (1,))
        next_X[1] = candidates[idx]

    print(f"\nQuery {t}/{NUM_QUERIES}:")
    print("A:", _fmt_car(next_X[0]))
    print("B:", _fmt_car(next_X[1]))

    # model's current prediction of user choice
...
attr_names  = ["Brand", "Color", "Fuel"]
attr_levels = [brands,  colors,  fuels]

with torch.no_grad():
    ls = model.covar_module.base_kernel.lengthscale.view(-1)  # (3,)
    inv_ls = 1.0 / ls
    print("\nLengthscale-based importance (bigger = more important):")
    for name, val in zip(attr_names, inv_ls):
        print(f"  {name}: {val.item():.3f}")


# enumerate all 3^3 = 27 possible cars as indices (0,1,2)
idx_vals = torch.cartesian_prod(
    torch.arange(3),  # Brand
    torch.arange(3),  # Color
    torch.arange(3),  # Fuel
)

all_X = idx_vals.to(dtype=dtype)

with torch.no_grad():
    post = model.posterior(all_X)
    mu = post.mean.view(-1) # (27,)

importance = {}
attr_info  = []

print("\n=== Posterior preferences by attribute level ===")
for dim, attr_name in enumerate(attr_names):
    levels      = attr_levels[dim]
    level_stats = []  # (mean_util, level_name) in original index order

    print(f"{attr_name}:")
    for level_idx, level_name in enumerate(levels):
        # mask cars where this attribute is fixed to this level
        mask = (idx_vals[:, dim] == level_idx)
        mean_util = mu[mask].mean().item()
        level_stats.append((mean_util, level_name))
        print(f"  {level_name:>7}: mean utility = {mean_util:.3f}")

    # importance by how much this attribute can move utility
    means_only = [m for m, _ in level_stats]
    spread = max(means_only) - min(means_only)
    importance[attr_name] = spread
    print(f" utility spread for {attr_name}: {spread:.3f}")

    sorted_levels = sorted(level_stats, key=lambda t: t[0], reverse=True)
    attr_info.append({
        "name": attr_name,
        "spread": spread,
...
# rank attributes by importance (bigger spread = more important)
print("\n=== Attribute importance (most to least) ===")
total_spread = sum(importance.values()) or 1.0

for info in sorted(attr_info, key=lambda d: d["spread"], reverse=True):
    name   = info["name"]
    spread = info["spread"]
    rel    = spread / total_spread

    print(f"\n{name}: spread={spread:.3f} (~{rel:.0%} of total)")
    order_str = " -> ".join(level_name for _, level_name in info["level_stats"])
    print("Inferred preference order: " + order_str)
