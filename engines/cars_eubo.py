# engines/cars_eubo.py

"""
Cars block using EUBO for pairwise query selection.
"""
...
class CarsEUBOTrial:
    """Trial container for the cars–EUBO block."""
    ...
    def __init__(
        ...
        self.left_display = CarsEUBOEngine.format_car(left_features)
        self.right_display = CarsEUBOEngine.format_car(right_features)

        self.model_p_left: Optional[float] = None
        self.model_p_right: Optional[float] = None


class CarsEUBOEngine:
    """Engine for the cars–EUBO block."""

    BRANDS = ["Benz", "Toyota", "Ford"]
    COLORS = ["Black", "White", "Red"]
    FUELS = ["Hybrid", "Gas", "Electric"]

    def __init__(self, block_label: str = "cars_eubo") -> None:
        self.block_label = block_label
    ...
    def next_trial(self) -> Optional[CarsEUBOTrial]:
        if self._done:
            return None
        ...
        acq = AnalyticExpectedUtilityOfBestOption(pref_model=self.model)
